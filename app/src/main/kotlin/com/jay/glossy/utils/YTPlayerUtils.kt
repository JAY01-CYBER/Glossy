...
import com.jay.glossy.innertube.NewPipeExtractor
import com.jay.glossy.innertube.YouTube
import com.jay.glossy.innertube.models.YouTubeClient
import com.jay.glossy.innertube.models.YouTubeClient.Companion.WEB_REMIX
import com.jay.glossy.innertube.models.response.PlayerResponse
import com.jay.glossy.innertube.strategy.ContentAwareFallbackStrategy
import com.jay.glossy.innertube.strategy.ContentHints
import com.jay.glossy.constants.AudioQuality
import com.jay.glossy.utils.YTPlayerUtils.MAIN_CLIENT
import com.jay.glossy.utils.YTPlayerUtils.validateStatus
import com.jay.glossy.utils.cipher.CipherDeobfuscator
import com.jay.glossy.utils.cipher.FunctionNameExtractor
import com.jay.glossy.utils.cipher.PlayerJsFetcher
import com.jay.glossy.utils.potoken.PoTokenGenerator
import com.jay.glossy.utils.potoken.PoTokenResult
...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        val isUploadedTrack = contentHints.isUploaded == true ||
            playlistId == "MLPT" || playlistId?.contains("MLPT") == true
        ...
        val effectiveHints = contentHints.copy(
            isExplicit = contentHints.isExplicit == true || isAgeRestrictedFromResponse,
        ...
        for ((clientIndex, client) in streamClients.withIndex()) {
            ...
            if (client.requirePoToken && poToken == null) {
                Timber.tag(logTag).d("Skipping client ${client.clientName} - requires a PoToken")
                continue
            }

            if (client == MAIN_CLIENT) {
                streamPlayerResponse = mainPlayerResponse
                Timber.tag(logTag).d("Reusing response from metadata client: ${client.clientName}")
            } else {
            ...
            if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                ...
                if (audioConfig == null) {
                    ...
                    if (audioConfig != null) {
                        Timber.tag(logTag).d("AudioConfig obtained from response of client: ${client.clientName}")
                    } else {
                ...
                if (clientIndex == streamClients.lastIndex) {
